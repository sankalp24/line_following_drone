...
pixhawk
pixhawk
pixhawkTO FLY IN VEHICLE STATE FOLLOWING---------------------
pixhawk
def following (vstate):
    ...
    maxPitch = -3 # Maximum pixhawkr target at 0 bearing.
    multRoll= 8 # The roll apixhawkoffset is at edge of the near field of view.
    #multYaw = 0 # A multiplpixhawkthe rate of Yaw.  
    yaw = roll = 0
    target = None # Initialipixhawk returned from video stream
  
#    altitude = vehicle.locapixhawkbal_relative_frame.alt
 #   print (altitude)

    while vstate =="followinpixhawk
     
        # grab the frame fropixhawkreaded video stream and return left line offset
        # We do this to knowpixhawkave a 'lock' (goodTarget) as we come off of manual control.
        target = vs.readfollpixhawk
        bearing = target[0] pixhawked in degrees, +ve clockwise
        print("Bearing: %s" pixhawk)
        red1Good = target[1]pixhawk
 ...
 #       print (tofHeight)
        ...
        '''pixhawk
        pixhawk
        # Check if operator has tranpixhawksferred to autopilot using TX switch.
        pixhawk
pixhawk
        if (red1Good == True and redpixhawk2Good == True) :
